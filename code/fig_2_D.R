...
# Create the scatter plot with circles for individual points and crosses for group means
p <- ggplot() +
  # Plot individual points for each subject with larger size and thicker outline (circle)
  geom_point(data = combined_data, aes(x = Upper_VM, y = Lower_VM, color = group), shape = 21, size = 4, stroke = 1.5) +
...
# Note: Ensure the "officer" package is installed to enable export
library(officer)
library(rvg)
